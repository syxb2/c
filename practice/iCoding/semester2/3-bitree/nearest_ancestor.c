 ...
 * @brief 假设二叉树采用二叉链表方式存储， root指向根结点，p所指结点和q所指结点为二叉树中的两个结点，编写一个计算它们的最近的共同祖先.提示：在完成本题时，可利用 path 函数获取p和q两个结点到根结点之间的路径，之后再计算两条公共路径得出最近的共同祖先。
 * @version 0.1
 * @date 2024-06-13
 *
...
BiTNode *nearest_ancestor(BiTree root, BiTNode *p, BiTNode *q) {
    // Stack *s1 = NULL; // 不能这么写，因为这么写的话没有为 s1 分配内存
    Stack s1;
    init_stack(&s1);
    Stack s2;
    init_stack(&s2);
