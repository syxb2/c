 ...
 * @version 0.1
 * @date 2024-06-09
 * @brief 其中 edges 指向图的邻接矩阵，矩阵元素为对应边上的权重(权重为自然数)。 * 定义图中0结点为入口结点； * 定义dom关系为：如果从图的入口结点（即0结点）到结点 n 的每条路径都经过结点 d ，则称结点 d 支配(dominate) n ，记为 d dom n。 * 根据该定义，每个结点支配它自己，且入口结点（即0结点）支配包括它自己在内的所有结点。 * 请使用上述数据结构，从标准输入读入一个图(该图为联通图)，请找出除入口结点外所有结点间的支配关系，并按下面示例样式输出： * 输入数据示例如下： * 5,6 * 0,1,95 * 0,2,41 * 1,3,99 * 1,4,67 * 2,4,95 * 3,4,99 * 其中第1行分别为图中结点的个数 5、图中边的个数 6；之后的6行分别是各边的信息，以第2行为例，其表示结点0到1之间有权重为95的有向边，方向为0至1。根据支配关系的定义，该图的支配关系有： * 0 dom 0 * 0 dom 1 * 0 dom 2 * 0 dom 3 * 0 dom 4 * 1 dom 1 * 1 dom 3 * 2 dom 2 * 3 dom 3 * 4 dom 4 * 因0结点支配所有结点，所以不输出该信息；将其它支配关系按结点编号从小到大的顺序输出如下： * (1#1)(1#3)(2#2)(3#3)(4#4) * 其中每一对括号表示一个支配关系，# 表示　dom。
 * @version 0.1
 * @date 2024-06-09
 * 
